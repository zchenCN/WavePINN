"""
Physics Informed Neural Networks for solving wave equations and
identify the wavespeed  with two time snapshot wavefield data
...
class PINN:
    ...
    def __init__(self, network, TX_init, U_init, TX_pde, TX_test, U_test, c=1.0):
        ...
        self.c = torch.tensor([2.0], requires_grad=True, dtype=torch.float64).to(self.device)
        self.c = torch.nn.Parameter(self.c)
        self.grads = GradientLayer(self.network)

        self.network.register_parameter('velocity', self.c)
        ...
        self.optimizer_adam = torch.optim.Adam(
            network.parameters(), 
            lr=0.001# 0.1 bad, 0.01 good, 0.001 good, 0.0001 bad
        )
    ...
    def loss(self, wp=0.000001, wi=1.0, wb=0.0):#0.00001
        ...
        if self.num_epoch % 100 == 0:
            ...
            self.c_list.append(torch.abs(self.c).item())
    ...
    def train(self, mode='lbfgs'):
        ...
        if mode == 'lbfgs':
            ...
            while self.num_epoch < self.max_num_epoch:
                self.optimizer_adam.step(self.loss)
