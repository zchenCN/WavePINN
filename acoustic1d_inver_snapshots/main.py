...
# num_pde = 100
# t_pde, x_pde = np.meshgrid(np.linspace(0, 1, num_pde), np.linspace(0, 1, num_pde))
# TX_pde = np.c_[t_pde.flatten()[:, np.newaxis], x_pde.flatten()[:, np.newaxis]]
...
fig3 = plt.figure()
plt.plot(pinn.c_list, 'r-')
plt.plot(np.ones(len(pinn.c_list)), 'b--')
plt.title('wavespeed')
plt.tight_layout()
plt.savefig('wavespeed.png', transparent=True)
plt.show()
# change t1 data
